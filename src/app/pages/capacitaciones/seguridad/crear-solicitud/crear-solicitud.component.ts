...
export class CrearSolicitudComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.perfiles = [
      ...
      {"id":6,"descripcion":"Administrador del Sistema"},
    ]
  ...
  cancelProject() {
    this.router.navigate(['/capacitaciones/seguridad/solicitud-list']).then(r => r);
  }
  ...
  createSolicitudUsuarios() {
    ...
    this.alertService.questionAlertConfirm(
      ...
      (result) => {
        if (result.value) {
          this.router.navigate(['/capacitaciones/seguridad/solicitud-list']).then(() => { });
            this.alertService.confirmAlert(`C&oacute;digo Solicitud <br><b style="font-size: 30px">Solicitud creada con exito</b>`, TYPE_ALERT.SUCCESS);
        } else {
