...
@Component({
  selector: 'app-acreditacion-details',
  templateUrl: './acreditacion-details.component.html',
  styleUrls: ['./acreditacion-details.component.scss']
})
export class AcreditacionDetailsComponent implements OnInit {
  public random = Math.floor(Math.random() * (9 - 1) + 1)
  arrCalificado: any[] = [
    {
      id: 1,
      idCategoria: 1,
  ...
  constructor(public activemodal: NgbActiveModal) {
    ...
    let contador = 0
    this.arrCalificado.map((x:any) => {
      if(x.califica == "Si" ){
        contador++
        if(contador == this.random) x.estado = '2'
      }
    })
    console.log(this.arrCalificado);
